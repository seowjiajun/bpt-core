...
export function OptionsPositionPanel({ legs }: Props) {
  ...
  // Aggregated row
  const agg = useMemo(() => {
    let delta = 0, gamma = 0, vega = 0, theta = 0, pnl = 0
    for (const l of legs) {
    ...
    <div className="panel">
      ...
      <div className="panel-body panel-body--flush">
        <table className="blotter-table">
          <thead>
            <tr>
              <th onClick={() => toggleSort('expiry')} className="th-sortable">Expiry{arrow('expiry')}</th>
              <th onClick={() => toggleSort('strike')} className="th-sortable num">Strike{arrow('strike')}</th>
              <th>Type</th>
              <th onClick={() => toggleSort('qty')} className="th-sortable num">Qty{arrow('qty')}</th>
              <th className="num">Entry</th>
              <th className="num">Mark</th>
              <th className="num">IV</th>
              <th onClick={() => toggleSort('delta')} className="th-sortable num">Δ{arrow('delta')}</th>
              <th className="num">Γ</th>
              <th className="num">V</th>
              <th className="num">Θ</th>
              <th onClick={() => toggleSort('unrealizedPnl')} className="th-sortable num">PnL{arrow('unrealizedPnl')}</th>
            </tr>
          </thead>
          <tbody>
            {sorted.map((l) => (
              <tr key={l.instrumentId}>
                ...
                <td className="num" style={{ color: 'var(--text-muted)' }}>{l.gamma.toFixed(6)}</td>
                <td className="num" style={{ color: 'var(--text-muted)' }}>{l.vega.toFixed(1)}</td>
            ...
            <tr className="options-agg-row">
              <td colSpan={7} style={{ textAlign: 'right', fontWeight: 600 }}>Portfolio</td>
              <td className={`num ${pnlClass(agg.delta)}`} style={{ fontWeight: 600 }}>{agg.delta.toFixed(3)}</td>
              <td className="num" style={{ color: 'var(--text-muted)', fontWeight: 600 }}>{agg.gamma.toFixed(6)}</td>
              <td className="num" style={{ color: 'var(--text-muted)', fontWeight: 600 }}>{agg.vega.toFixed(1)}</td>
              <td className={`num ${pnlClass(agg.theta)}`} style={{ fontWeight: 600 }}>{agg.theta.toFixed(1)}</td>
