...
// Per-instrument row from the SBE positions group. Qty + entry price
// preserved — needed by callers that seed PositionTracker from the
// snapshot (reconciler-driven resync). Callers that only need qty
// should use `extract_exchange_positions` above; it's slightly
