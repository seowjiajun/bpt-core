...
PassiveMakerStrategy::PassiveMakerStrategy(uint64_t correlation_id,
    ...
    : correlation_id_(correlation_id),
      ...
      vol_window_size_(static_cast<std::size_t>(
          cfg.params["vol_window_size"].value<int64_t>().value_or(60))),
      vol_sample_interval_ns_(static_cast<uint64_t>(
          cfg.params["vol_sample_interval_ms"].value<double>().value_or(1000.0) * 1e6)),
      regime_gating_enabled_(
          cfg.params["regime_gating_enabled"].value<bool>().value_or(false)),
      regime_cfg_{
          static_cast<std::size_t>(
              cfg.params["regime_window_size"].value<int64_t>().value_or(60)),
          static_cast<uint64_t>(
              cfg.params["regime_sample_interval_ms"].value<double>().value_or(1000.0) * 1e6),
          ...
          static_cast<uint64_t>(
              cfg.params["regime_chop_cooldown_s"].value<double>().value_or(120.0) * 1e9),
      },
      vol_gate_cfg_{
    ...
    bpt::common::log::info(kLog(),
        ...
        half_spread_bps_, inventory_penalty_, requote_threshold_bps_,
        qty_per_quote_, max_inventory_);
    bpt::common::log::info(kLog(),
        "linear_vol_scaling: spread_mult={:.2f} requote_mult={:.2f} "
        "vol_window={} sample_interval={}ms",
        spread_vol_mult_, requote_vol_mult_,
        vol_window_size_, vol_sample_interval_ns_ / 1'000'000);
...
void PassiveMakerStrategy::on_snapshot(const refdata::InstrumentCache& cache) {
    ...
    for (uint64_t id : all_ids) {
        ...
        if (inst->exchange == "BINANCE")          ex_id = ExchangeId::BINANCE;
        else if (inst->exchange == "OKX")         ex_id = ExchangeId::OKX;
        else if (inst->exchange == "HYPERLIQUID") ex_id = ExchangeId::HYPERLIQUID;
        else if (inst->exchange == "DERIBIT")     ex_id = ExchangeId::DERIBIT;

        InstrumentState st(vol_gate_cfg_, vol_window_size_, vol_sample_interval_ns_,
                           regime_cfg_);
        st.instrument_id = id;
    ...
    for (const auto& [id, st] : state_)
        ...
        subs.push_back({id, st.exchange, st.symbol, /*depth=*/1});
...
void PassiveMakerStrategy::on_bbo(const bpt::messages::MdMarketData& tick) {
    ...
    if (st.vol_gate.update_and_check(mid, st.last_book_ns)) {
        bpt::common::log::info(kLog(), "{} vol_gate halted — cancelling resting quotes",
                       st.symbol);
    ...
    const bool inventory_breach =
        ...
        ((st.inventory > 0 && st.bid_order_id != 0) ||
         (st.inventory < 0 && st.ask_order_id != 0));
...
bool PassiveMakerStrategy::drift_exceeds_threshold(const InstrumentState& st,
                                                    double fv_now) const {
    ...
    const double rv_per_min_bps = st.vol_est.ready()
        ? annualized_to_per_minute_bps(st.vol_est.realized_vol())
        : 0.0;
    const double effective_threshold =
        scale_with_vol(requote_threshold_bps_, requote_vol_mult_, rv_per_min_bps);
...
void PassiveMakerStrategy::requote(InstrumentState& st, double fv, uint64_t /*now_ns*/) {
    ...
    const double rv_per_min_bps = st.vol_est.ready()
        ? annualized_to_per_minute_bps(st.vol_est.realized_vol())
        : 0.0;
    const double effective_half_bps =
        scale_with_vol(half_spread_bps_, spread_vol_mult_, rv_per_min_bps);
    ...
    bpt::common::log::info(kLog(),
        ...
        st.symbol, st.inventory, fv, bid_px, ask_px,
        effective_half_bps, half_spread_bps_, rv_per_min_bps);
}

uint64_t PassiveMakerStrategy::place_side(InstrumentState& st,
                                          OrderSide::Value side, double price) {
    if (!order_mgr_)
        return 0;
    ...
    const double qty = bpt::strategy::venue::bump_qty_for_min_notional(
        qty_per_quote_, price, st.lot_size,
    ...
    const uint64_t oid = order_mgr_->place_order(
        st.instrument_id, st.exchange_id, side,
        OrderType::LIMIT, TimeInForce::GTC,
        price, qty);
    if (oid == 0) {
        bpt::common::log::warn(kLog(), "{} place_order rejected ({} @ {:.6f})",
                       st.symbol,
    ...
    if (status == ExecStatus::PARTIAL || status == ExecStatus::FILLED) {
        ...
        if (cum_filled_e8 > last_e8) {
            ...
            const double signed_delta =
                (rpt.side() == bpt::messages::OrderSide::BUY) ? delta_qty : -delta_qty;
            ...
            bpt::common::log::info(kLog(),
                ...
                delta_qty, delta_px, st.inventory);
        }
    }

    const bool is_terminal = (status == ExecStatus::FILLED) ||
                             (status == ExecStatus::CANCELLED) ||
                             (status == ExecStatus::REJECTED);

    if (is_terminal) {
        if (st.bid_order_id == order_id) st.bid_order_id = 0;
        if (st.ask_order_id == order_id) st.ask_order_id = 0;
        ...
        if (order_mgr_) {
            ...
            if (std::abs(st.inventory) > 1e-9) {
                ...
                if (mid > 0.0) {
                    ...
                    const double price = (side == OrderSide::SELL)
                                              ? st.bid * (1.0 - 5.0 / 1e4)
                                              : st.ask * (1.0 + 5.0 / 1e4);
                    const uint64_t oid = order_mgr_->place_order(
                        st.instrument_id, st.exchange_id, side,
                        OrderType::LIMIT, TimeInForce::IOC,
                        price, std::abs(st.inventory));
                    if (oid != 0)
                        order_to_instrument_[oid] = st.instrument_id;
                    bpt::common::log::warn(kLog(),
                        ...
                        std::abs(st.inventory), price, st.inventory);
                }
            }
        }
    }
