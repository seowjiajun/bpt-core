...
namespace {
// Sub-module logger — auto-prefixed with "HMM" via %(logger). Lazy-init
// because bpt::common::logging::init() runs after static initialisation.
quill::Logger* kLog() {
    static quill::Logger* l = bpt::common::logging::get_logger("HMM");
    return l;
}
}  // namespace
...
HmmStrategy::HmmStrategy(uint64_t correlation_id,
    ...
    bpt::common::log::info(kLog(),
                           "confidence={:.2f} min_dwell={} bar={:.1f}s depth={}",
                           confidence_threshold_,
                           min_dwell_bars_,
                           bar_interval_ns_ / 1e9,
                           order_book_depth_);
    bpt::common::log::info(kLog(),
                           "momentum: ema={}/{} atr={} stop={:.1f}x target={:.1f}x qty_usd={:.0f}",
                           ema_fast_period_,
                           ema_slow_period_,
                           atr_period_,
                           atr_stop_mult_,
                           ...
                           momentum_qty_usd_);
    bpt::common::log::info(kLog(),
                           "reversion: dev={:.1f}x_atr close={:.1f}x_atr stop={:.1f}x_atr qty_usd={:.0f}",
                           vwap_deviation_atr_,
                           vwap_close_atr_,
                           vwap_stop_atr_,
                           reversion_qty_usd_);
    bpt::common::log::info(kLog(),
                           "mm: gamma={:.2f} k={:.2f} horizon={:.1f}s qty_usd={:.0f} "
...
void HmmStrategy::evaluate_regime(InstrumentState& st) {
    ...
    bpt::common::log::info(kLog(),
                           "{} regime {} → {} (conf={:.3f} α=[{:.2f},{:.2f},{:.2f},{:.2f}] dwell={})",
                           st.symbol,
                           HmmFilter::state_name(st.regime),
...
void HmmStrategy::check_transition_complete(InstrumentState& st) {
    ...
    if (net != 0) {
        ...
        if (st.close_order_id != 0) {
            ...
            bpt::common::log::info(kLog(),
                                   "{} closing position (net={}) before entering {}",
                                   st.symbol,
                                   net,
    ...
    bpt::common::log::info(kLog(), "{} now in regime {}", st.symbol, HmmFilter::state_name(st.regime));
...
void HmmStrategy::momentum_check_signal(InstrumentState& st) {
    ...
    bpt::common::log::info(kLog(),
                           "{} momentum {} signal: ema_fast={:.4f} ema_slow={:.4f} atr={:.4f}",
                           st.symbol,
                           trend_up ? "BUY" : "SELL",
                           st.ema_fast,
                           st.ema_slow,
...
void HmmStrategy::momentum_check_exit(InstrumentState& st, double mid) {
    ...
    if (should_exit) {
        bpt::common::log::info(kLog(),
                               "{} momentum exit: {} mid={:.2f} stop={:.2f} target={:.2f}",
                               st.symbol,
                               reason,
                               mid,
                               st.momentum_stop,
...
void HmmStrategy::reversion_check_signal(InstrumentState& st) {
    ...
    bpt::common::log::info(kLog(),
                           "{} vwap reversion {}: mid={:.4f} vwap={:.4f} dev={:.4f} "
                           "thr={:.4f} stop={:.4f}",
                           st.symbol,
...
void HmmStrategy::reversion_check_exit(InstrumentState& st, double mid) {
    ...
    if (should_exit) {
        bpt::common::log::info(kLog(), "{} reversion exit: {} mid={:.4f} vwap={:.4f}", st.symbol, reason, mid, wvap);
...
void HmmStrategy::mm_quote(InstrumentState& st) {
    ...
    bpt::common::log::debug(kLog(),
                            "{} mm: bid={:.4f} ask={:.4f} res={:.4f} hs={:.4f} q={:.6f} σ_T={:.6f}",
                            st.symbol,
                            new_bid,
                            new_ask,
                            reservation,
    ...
    if (status == ExecStatus::REJECTED) {
        ...
        if (!gw_reject) {
            if (++st.consecutive_rejects >= 3) {
                ...
                bpt::common::log::warn(kLog(),
                                       "{} 30s reject cooldown after {} consecutive rejects",
                                       st.symbol,
        ...
        if (order_id == st.momentum_order_id) {
            if (status == ExecStatus::FILLED) {
                if (st.has_momentum_position) {
                    ...
                    bpt::common::log::info(kLog(),
                                           "{} momentum entered {} entry={:.4f} stop={:.4f} target={:.4f}",
                                           st.symbol,
        ...
        if (order_id == st.reversion_order_id) {
            if (status == ExecStatus::FILLED) {
                if (st.has_reversion_position) {
                    ...
                    bpt::common::log::info(kLog(), "{} reversion position closed", st.symbol);
                } else {
                    st.has_reversion_position = true;
                    bpt::common::log::info(kLog(), "{} reversion entered", st.symbol);
                }
            } else {
                st.reversion_order_id = 0;
            }
