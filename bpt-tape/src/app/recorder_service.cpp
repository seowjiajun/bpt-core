...
RecorderService::RecorderService(config::Settings settings, const bpt::common::util::Topology& topology)
...
void RecorderService::setup_metrics() {
    ...
    if (settings_.base.metrics_port == 0)
        return;

    metrics_ = std::make_unique<metrics::TapeMetrics>(settings_.metrics_host, settings_.base.metrics_port);
    bpt::common::log::info("bpt-tape: metrics exposer at {}:{}", settings_.metrics_host, settings_.base.metrics_port);
}

std::shared_ptr<bpt::tape::io::Tape> RecorderService::make_tape(const std::string& venue_tag) {
    return std::make_shared<bpt::tape::io::Tape>(bpt::tape::io::Tape::Config{
        ...
        .flush_interval_ns = static_cast<uint64_t>(settings_.recording.fsync_interval_ms) * 1'000'000ULL,
        .metrics = metrics_ ? metrics_->hooks_for(venue_tag) : bpt::tape::io::Tape::MetricsHooks{},
    });
}

void RecorderService::wire_connection_markers(std::shared_ptr<bpt::md_gateway::adapter::IAdapter> adapter,
    ...
    adapter->on_connect = [tape, state, metrics, venue_tag]() {
        if (metrics)
            metrics->on_ws_connect(venue_tag);
        if (!state->was_disconnected)
            return;  // initial connect — SESSION_START covers it
    ...
    adapter->on_disconnect = [tape, state, metrics, venue_tag]() {
        if (metrics)
            metrics->on_ws_disconnect(venue_tag);
        tape->write_marker(WallClock::now_ns(),
                           bpt::common::recorder::RecordType::WS_DISCONNECT,
                           fmt::format(R"({{"attempt":{}}})", state->reconnect_count + 1));
        tape->flush();
    ...
    for (const auto& a_cfg : settings_.mdgw_adapters) {
        ...
        if (!exch_id) {
            throw std::runtime_error(
                fmt::format("Unknown exchange '{}' in bpt-tape config — not in messages/exchanges.yaml",
                            a_cfg.exchange));
        }
        auto adapter = adapter::make_recording_adapter<NoopMdPublisher>(*exch_id, tape, a_cfg, pub);
        if (!adapter) {
            throw std::runtime_error(
                fmt::format("Exchange '{}' is in the registry but bpt-tape has no recording adapter for it",
                            a_cfg.exchange));
        }
        ...
        bpt::common::log::info("bpt-tape: started recording adapter for {} → {}", a_cfg.exchange, tape->current_path());
    }
}
    ...
    try {
        ...
        const auto matches_filter = [&filter](const auto& entry) {
            if (!filter.inst_types.empty()) {
                if (std::find(filter.inst_types.begin(), filter.inst_types.end(), entry.info.type) ==
                    filter.inst_types.end())
                    return false;
            }
            if (std::find(filter.exclude_bases.begin(), filter.exclude_bases.end(), entry.info.base) !=
                filter.exclude_bases.end())
                return false;
            return true;
        };
        ...
        for (const auto& [venue_name, adapter] : adapters_per_venue_) {
            ...
            if (!venue_id)
                continue;  // unreachable: ctor validates above
...
void RecorderService::setup_refdata_pollers() {
    ...
    // converter's input.
    std::unordered_map<std::string, std::vector<refdata::EndpointSpec>> endpoints_per_venue;
    ...
    for (auto& [venue_name, eps] : endpoints_per_venue) {
        ...
        tape->write_marker(WallClock::now_ns(),
                           ...
                           fmt::format(R"({{"pid":{},"exchange":"{}","kind":"refdata","endpoints":{}}})",
                                       ...
                                       eps.size()));
        tape->flush();
        auto poller = std::make_unique<refdata::RefdataPoller>(venue_tag, tape, std::move(eps));
        poller->start();
...
void RecorderService::stop() {
    ...
    for (auto& a : adapters_)
        a->stop();
    for (auto& s : tapes_) {
        s->write_marker(WallClock::now_ns(), bpt::common::recorder::RecordType::SESSION_STOP, R"({"reason":"stop"})");
        s->flush();
    }
    for (auto& p : refdata_pollers_)
        p->stop();
