...
TapeMetrics::TapeMetrics(const std::string& host, uint16_t port) {
    ...
    wslog_rotation_failures_total_fam_ =
        &prometheus::BuildCounter()
             ...
             .Help(
                 "Rotation-open failures per (venue, cause). cause is one of "
    ...
    ws_connected_fam_ = &prometheus::BuildGauge()
                             .Name("bpt_tape_ws_connected")
                             .Help("1 if the venue WebSocket is currently connected, 0 otherwise.")
                             .Register(*registry_);

    ws_reconnects_total_fam_ = &prometheus::BuildCounter()
                                    .Name("bpt_tape_ws_reconnects_total")
                                    .Help(
                                        "Cumulative (re)connect count per venue. Includes the bootstrap "
                                        "connect; alert TapeWsFlapping keys on rate() over a 5m window.")
                                    .Register(*registry_);

    subscriptions_fam_ = &prometheus::BuildGauge()
                              .Name("bpt_tape_subscriptions")
                              .Help(
                                  "Count of currently-subscribed instruments per venue. Set once "
...
bpt::tape::io::Tape::MetricsHooks TapeMetrics::hooks_for(const std::string& venue) {
    ...
    return bpt::tape::io::Tape::MetricsHooks{
        .on_write_success =
            [&last_write, &frames, &bytes](uint64_t recv_ts_ns, std::size_t total_bytes) {
                last_write.Set(static_cast<double>(recv_ts_ns) / 1e9);
                frames.Increment();
                bytes.Increment(static_cast<double>(total_bytes));
            },
        .on_rotation_success = [&rotations]() { rotations.Increment(); },
        .on_rotation_failure =
            [failures_fam, venue](std::string_view cause) {
                failures_fam->Add({{"venue", venue}, {"cause", std::string(cause)}}).Increment();
            },
    };
}
