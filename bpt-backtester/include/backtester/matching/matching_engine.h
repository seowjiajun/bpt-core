...
public:
    ...
    // Optional latency model. When null (default), submit_to_match and
    // match_to_report delays are zero — pre-Phase-3 behaviour. When set,
    // every submitted order's match is deferred until current_ts_ ≥
    // submitted_ts + draw(SUBMIT_TO_MATCH); every produced fill's delivery
    // to fill_cb_ is deferred until current_ts_ ≥ match_ts + draw(MATCH_TO_REPORT).
    ...
    // Called from the order WS/REST server when OrderGateway submits an
    // order. POST_ONLY orders that would cross the current book are
    // synchronously rejected (real exchanges return this in the ack);
    // everything else returns ACCEPTED immediately and the actual match
    // is deferred per the latency model. Fills come later via fill_cb_.
    OpenOrder submit_order(OpenOrder order);

    // Returns true if the order was found and cancelled. Scans both
    // pending_submits_ (orders waiting on submit_to_match) and pending_
    // (resting limits awaiting fills).
...
private:
    ...
    // Runs the actual matching logic for a single order at its scheduled
    // match time. Equivalent to the pre-Phase-3 body of submit_order
    // (minus the synchronous POST_ONLY-cross check, which fires earlier).
    // Caller must hold mutex_.
    void process_pending_submit(PendingSubmit& ps, std::vector<FillReport>& out);

    // Pops all pending submits with scheduled_match_ts ≤ upto_ts, in
    // chronological order, processing each through the match logic.
    // Produced fills are scheduled for delivery via schedule_fill.
    // Caller must hold mutex_.
    void drain_pending_submits(uint64_t upto_ts);

    // Schedules a fill for delivery at fill.simulation_ts + match_to_report.
    // Caller must hold mutex_.
    void schedule_fill(FillReport fill);

    // Pops all pending fills with scheduled_report_ts ≤ upto_ts and
    // appends them to out. Caller must hold mutex_; fill_cb_ fires
    // outside the lock by the caller.
    void drain_pending_fills(uint64_t upto_ts, std::vector<FillReport>& out);

    // Phase 5: when a book update arrives, attribute the volume drop at
    // each price level to (cancels) = (Δ size) − (trade volume since last
    // book update at that level). Decrement queue_ahead on resting orders
    // proportionally to the cancel share, under the uniform-cancel
    // approximation (each unit ahead has equal probability of being a
    // cancel). Caller must hold mutex_; called between drain_pending_submits
    // and the books_[key] write.
    void apply_queue_regen(const std::string& book_key, const data::OrderBookRecord& new_book);

    // Discrete key for a price level. Backed by round(price * 1e9) so we
    // dodge double-keyed unordered_map collision risk while keeping
    // lookups exact for venue-tick-aligned prices.
