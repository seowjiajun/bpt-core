...
/// \brief Collects fills and market prices throughout a backtest run and writes
...
public:
    /// \brief Identity fields the archive list can use to differentiate runs without
    ///        opening the detail view.
    ///
    /// simulation_start/end are ISO strings copied straight from the
    /// simulation config. wallclock_start_ns is captured here at
    /// construction; wallclock_duration_ms is computed at write() time.
    struct RunMetadata {
    ...
    /// \brief Compose a deterministic run_id from the metadata + window.
    ///
    /// Used for the on-disk output directory and as the primary key in
    /// archive tooling. Falls back to "{start}_{end}" if the identity
    /// fields are empty so older runs and ad-hoc invocations still produce
    /// a path.
    ...
    /// \brief Called on every market event from ClockMaster.
    ///
    /// Used to keep a mark-to-market mid price per symbol for unrealized PnL.
    void on_market_event(const data::MarketEvent& event);

    /// \brief Write all output files.  Creates output_dir if it does not exist.
    void write() const;

    /// \brief Exposed for testing.
...
private:
    ...
    /// \brief Apply a fill to the position, return realized PnL from any closed portion.
    ...
    /// \brief Pending markout resolutions — populated on each fill, drained on
    ///        each subsequent market event whose ts crosses the target.
    ///
    /// Linear scan is fine: the queue stays small (≤ 4 × in-flight unresolved
