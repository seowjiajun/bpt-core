...
/// \file
/// \brief Backtester configuration types loaded from TOML.
...
/// \brief One entry per instrument Backtester will replay.
///
...
/// \brief Localhost WS ports that MdGateway and OrderGateway connect to instead of real exchanges.
...
/// \brief Per-venue, per-leg simulated latency.
///
/// base_ns is the floor; jitter_ns adds a uniform [0, jitter_ns) draw on
/// top, seeded once per run for reproducibility (see SimLatencyConfig::seed).
...
/// \brief One half-open replay window.
///
/// Multiple windows can be unioned via [[simulation.windows]] — useful for
/// "every Asian open", "all FOMC ±30s", or stitched walk-forward slices.
struct TimeWindow {
...
struct ResultsConfig {
    ...
    /// \brief Per-venue maker/taker fee rates in basis points of notional.
    ///
    /// Looked up by FillReport.exchange + FillReport.liquidity_role, so a single
...
/// \brief Aeron streams used for backtest tick-gating.
///
