...
matching::OrderSide to_match_side(bpt::messages::OrderSide::Value s) {
    return s == bpt::messages::OrderSide::BUY ? matching::OrderSide::BUY
                                              : matching::OrderSide::SELL;
}

bpt::messages::OrderSide::Value to_msg_side(matching::OrderSide s) {
    return s == matching::OrderSide::BUY ? bpt::messages::OrderSide::BUY
                                          : bpt::messages::OrderSide::SELL;
}

matching::OrderType to_match_type(bpt::messages::OrderType::Value t, bool post_only) {
    if (post_only) return matching::OrderType::POST_ONLY;
    if (t == bpt::messages::OrderType::MARKET) return matching::OrderType::MARKET;
    return matching::OrderType::LIMIT;
}

bpt::messages::OrderType::Value to_msg_type(matching::OrderType t) {
    switch (t) {
        case matching::OrderType::MARKET:    return bpt::messages::OrderType::MARKET;
        case matching::OrderType::LIMIT:     return bpt::messages::OrderType::LIMIT;
        case matching::OrderType::POST_ONLY: return bpt::messages::OrderType::LIMIT;  // wire-level
    }
...
std::string InProcessOrderGatewayClient::exchange_id_string(bpt::messages::ExchangeId::Value id) {
    ...
    switch (id) {
        case EX::BINANCE:     return "BINANCE";
        case EX::OKX:         return "OKX";
        case EX::HYPERLIQUID: return "HYPERLIQUID";
        case EX::DERIBIT:     return "DERIBIT";
        default:              return "";
    }
}

InProcessOrderGatewayClient::InProcessOrderGatewayClient(matching::MatchingEngine& matching)
    : matching_(matching) {
...
bool InProcessOrderGatewayClient::send_new_order(uint64_t order_id,
    ...
    if (order_type != bpt::messages::OrderType::MARKET && price <= 0) return false;
    if (exchange_symbol.empty()) return false;
    ...
    if (it != live_.end()) {
        ...
        if (cumfilled < quantity) {
            publish_exec_status(order_id, exchange_id, instrument_id,
                                ...
                                side, order_type, price, quantity, cumfilled);
        }
    }
    return true;
}
...
void InProcessOrderGatewayClient::send_cancel(uint64_t order_id,
    ...
    if (ok) {
        publish_exec_status(order_id, exchange_id, instrument_id,
                            ...
                            lo.side, lo.order_type, lo.price, lo.quantity,
...
void InProcessOrderGatewayClient::send_account_snapshot_request(
    ...
    if (!on_account_snapshot) return;
    constexpr std::size_t kBufSize =
        bpt::messages::MessageHeader::encodedLength() +
        bpt::messages::AccountSnapshot::sbeBlockLength();
    char buf[kBufSize]{};
...
void InProcessOrderGatewayClient::push_heartbeat() {
    ...
    if (!on_heartbeat) return;
    constexpr std::size_t kBufSize =
        bpt::messages::MessageHeader::encodedLength() +
        bpt::messages::OrderGatewayHeartbeat::sbeBlockLength();
    char buf[kBufSize]{};
...
void InProcessOrderGatewayClient::publish_fill(const matching::FillReport& fr) {
    ...
    const auto status = fr.is_fully_filled
    ...
    publish_exec_status(order_id, lo.exchange_id, lo.instrument_id,
                        status, to_msg_side(fr.side), to_msg_type(fr.order_type),
                        fill_px_scaled, fill_qty_scaled,
                        lo.cumulative_filled_qty);
...
void InProcessOrderGatewayClient::publish_exec_status(
    uint64_t order_id,
    bpt::messages::ExchangeId::Value exchange_id,
    uint64_t instrument_id,
