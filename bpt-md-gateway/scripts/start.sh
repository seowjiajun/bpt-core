...
#
# Config comes from $BPT_MD_GATEWAY_CONFIG in deploy/env/active.env, which the
...
UNIT="bpt-md-gateway.service"

if [[ $# -gt 0 ]]; then
    echo "NOTE: $0 is a systemd wrapper; config arg ignored." >&2
    echo "      Config comes from deploy/env/active.env (\$BPT_MD_GATEWAY_CONFIG)." >&2
