...
AeronBus AeronBus::build(std::shared_ptr<aeron::Aeron> aeron, const config::Settings& settings) {
    AeronBus bus;
    bus.control_source =
        std::make_unique<MdControlSubscriber>(aeron, settings.aeron.control.channel, settings.aeron.control.stream_id);
    bus.md_sink = std::make_shared<MdPublisher>(aeron, settings.aeron.data.channel, settings.aeron.data.stream_id);
    bus.ack_sink =
        std::make_unique<AckPublisher>(aeron, settings.aeron.ack_hb.channel, settings.aeron.ack_hb.stream_id);
