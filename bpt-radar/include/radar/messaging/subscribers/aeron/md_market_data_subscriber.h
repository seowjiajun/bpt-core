...
public:
    MdMarketDataSubscriber(std::shared_ptr<::aeron::Aeron> aeron, const bpt::common::config::StreamConfig& stream) {
        sub_ = bpt::common::aeron::wait_for_subscription(aeron, stream.channel, stream.stream_id);
        bpt::common::log::info("[MdMarketDataSubscriber] ready on stream {}", stream.stream_id);
    }
