...
AeronBus AeronBus::build(std::shared_ptr<aeron::Aeron> aeron,
    ...
    AeronBus bus;
    bus.control_source = std::make_shared<OrderSubscriber>(
        aeron, settings.aeron.order.channel, settings.aeron.order.stream_id);
    bus.exec_sink = std::make_shared<ExecReportPublisher>(
        aeron, settings.aeron.exec_report.channel, settings.aeron.exec_report.stream_id);
    bus.account_snapshot_sink = std::make_shared<AccountSnapshotPublisher>(
        aeron, settings.aeron.account_snapshot.channel, settings.aeron.account_snapshot.stream_id);
    bus.heartbeat_sink = std::make_shared<HeartbeatPublisher>(
        aeron, settings.aeron.heartbeat.channel, settings.aeron.heartbeat.stream_id);
    return bus;
}
