...
void DeribitWsClient::on_handshake_complete() {
    if (login_msg_builder_)
        send(login_msg_builder_());
}

void DeribitWsClient::on_frame(std::string_view payload, uint64_t recv_ns) {
    if (message_handler_)
        message_handler_(std::string(payload), recv_ns);
}

void DeribitWsClient::run(std::atomic<bool>& stop_flag, std::atomic<bool>& connected) {
    bpt::common::log::info("DeribitWsClient connecting {}:{}{}", cfg_.ws_host, cfg_.ws_port, cfg_.ws_path);

    auto ws_ptr = bpt::common::ws::ws_connect(ioc_,
                                              ssl_ctx_,
                                              cfg_.ws_host,
                                              cfg_.ws_port,
                                              cfg_.ws_path,
