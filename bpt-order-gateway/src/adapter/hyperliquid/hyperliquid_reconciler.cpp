...
bool side_matches(const boost::json::object& o, bpt::messages::OrderSide::Value expected) {
    auto it = o.find("side");
...
int64_t to_e8(double v) {
    return static_cast<int64_t>(std::round(v * kScale));
}
...
bool open_order_matches(const boost::json::object& o,
    ...
    if (coin_of(o) != c.exchange_symbol)
        return false;
    if (!side_matches(o, c.side))
        return false;

    const int64_t sz_e8 = to_e8(parse_decimal_string(o, "sz"));
    if (sz_e8 != static_cast<int64_t>(c.quantity_e8))
        return false;
...
// oid mapping.
bool user_fill_matches(const boost::json::object& o, const HyperliquidReconciler::Candidate& c, int64_t price_tick_e8) {
    if (coin_of(o) != c.exchange_symbol)
        return false;
    if (!side_matches(o, c.side))
        return false;
...
HyperliquidReconciler::MatchResult HyperliquidReconciler::try_match(const Candidate& c,
    ...
    if (fill_match_count == 1) {
        ...
        if (auto it = fill_match->find("time"); it != fill_match->end() && it->value().is_int64())
...
void HyperliquidReconciler::worker_loop() {
    while (!stop_.load(std::memory_order_acquire)) {
        ...
        {
            std::unique_lock<std::mutex> lock(mu_);
            cv_.wait_for(lock, grace_period_, [this] { return stop_.load(std::memory_order_acquire); });
            if (stop_.load(std::memory_order_acquire))
                return;
        }
        ...
        try {
            ...
            bpt::common::log::warn("HL reconciler: poll failed for client_id={}: {}", c.client_order_id, e.what());
