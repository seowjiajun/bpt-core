...
const char* td_mode_for(std::string_view inst_id) {
    if (inst_id.find("-SWAP") != std::string_view::npos || inst_id.find("-FUTURES") != std::string_view::npos) {
        return "cross";
    }
    return "cash";
}
...
json::value build_cancel_action(std::string_view inst_id, std::string_view cloid, uint64_t req_id) {
    json::object arg;
