...
/// \file
/// \brief Hyperliquid L1 signer — secp256k1 / EIP-712 over msgpack(action) + nonce.
...
/// \brief Signed Hyperliquid transaction — returned by HyperliquidSigner.
///
/// `r`/`s` are 32-byte hex strings with NO `0x` prefix. `v` is 27 or 28
/// (the recovery id + 27, as Hyperliquid expects).
...
/// \brief Holds the private key and signs L1 actions.
///
...
///      where `vault_byte = 0x00` if no vault, else `0x01 || vault_address(20)`,
///      and `expires_byte = 0x00` if no expires, else `0x01 || expires_BE_8`.
...
///      \endcode
///   4. Sign `keccak256("\x19\x01" || domainSep || structHash)` via
///      secp256k1 with a canonical-low-s signature, recover id → v ∈
///      {27, 28}.
///
/// **Security**:
///   - Key is passed in as a hex string (with or without `0x` prefix).
///   - Key bytes live in a member array zeroed on destruction.
///   - No method exposes the key bytes; no intermediate values are logged.
///   - Final class, non-copyable, non-movable.
class HyperliquidSigner final {
public:
    /// \param hex         64-char secp256k1 private key (optionally 0x-prefixed).
    /// \param is_mainnet  Determines the "source" field in the Agent struct
    ...
    /// \brief Sign an L1 action (e.g. `{"type":"order",...}` or `{"type":"cancel",...}`).
    ///
    ...
    /// \brief Returns a monotonic, unique nonce suitable for use with Hyperliquid.
    ///
