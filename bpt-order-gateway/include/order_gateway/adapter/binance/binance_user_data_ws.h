...
/// \file
/// \brief Binance user-data WS client — owns the listenKey lifecycle and the read loop.
///
...
/// WS endpoint uses the listenKey as a path suffix —
/// `wss://.../ws/<key>`.
///
/// When the listenKey REST endpoint is unavailable (e.g. testnet 410
/// error), run() falls back to REST-only mode: it parks on a 1 s sleep
...
public:
    ...
    ///
    /// Creates a listenKey, connects the WS, runs the read loop until
    /// `stop_flag` is set, then deletes the listenKey. Sets `connected`
    /// to true after the handshake; base class clears it on return.
    ///
    /// If the listenKey endpoint returns empty (testnet 410 etc.), runs
    /// in REST-only fallback: sleeps 1 s in a loop with connected=true
