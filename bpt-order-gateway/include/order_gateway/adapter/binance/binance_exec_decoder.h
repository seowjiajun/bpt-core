...
/// \file
/// \brief Binance exec event decoder (WS executionReport + REST response → ExecEvent).
...
/// \brief Decodes Binance executionReport WS events into ExecEvents.
///
/// Owns the cloid→order_id map so both the WS handler and
/// send_new_order can register/look up client order IDs without locking
/// from two places.
...
public:
    ...
    /// \brief Called for each `executionReport` event on the user-data WebSocket.
    void handle_execution_report(const boost::json::object& obj, uint64_t recv_ns);

    /// \brief Decode the parsed JSON body of a `POST /api/v3/order` response.
    ///
