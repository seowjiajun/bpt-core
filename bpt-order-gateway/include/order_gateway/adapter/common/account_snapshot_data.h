...
// Per-currency cash balance row. OKX's /account/balance carries a
// details[] array with one entry per currency the account holds. The
// single aggregated `available_balance_e8` collapses this into USDT only;
// this struct preserves the per-ccy breakdown so the dashboard can show
// a row per currency (USDT, USDC, SGD, USD, BTC, etc.).
