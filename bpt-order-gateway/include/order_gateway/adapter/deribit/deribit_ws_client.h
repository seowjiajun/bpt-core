...
/// \file
/// \brief Persistent WebSocket client for Deribit's JSON-RPC endpoint.
///
/// Owns one connection session's read loop and thread-safe send.
/// Inherits the connect/read/send scaffolding from
/// `bpt::common::ws::RunLoop`; this class only implements the
/// Deribit-specific bits:
...
public:
    ...
    ///
    /// Blocks on the read loop until `stop_flag` goes true or the
    /// connection throws. Sets `connected` right after handshake
    /// completes (before the auth reply arrives).
