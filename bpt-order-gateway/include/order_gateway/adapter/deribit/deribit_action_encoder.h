...
/// \file
/// \brief Deribit WebSocket action JSON-RPC builders.
///
/// Pure transformations from SBE protocol values to the
/// `{"jsonrpc":"2.0",...}` shape Deribit's WS API expects. No state, no
/// I/O. Each helper takes the req_id explicitly so the adapter's
/// monotonic counter stays in one place.
///
/// Deribit's order API lives entirely over WebSocket — there is no REST
/// fallback — so the codec produces the final serialised string for the
/// caller to write directly to the socket.
...
///
/// Limit orders carry price + time_in_force; market orders omit both.
[[nodiscard]] std::string build_new_order_msg(const OrderSpec& spec, uint64_t req_id);

/// \brief `private/cancel` by exchange order id.
///
/// Deribit doesn't cancel by client label — caller must look up the
/// exchange id via the exec decoder.
...
/// \brief Simple JSON-RPC wrapper: `{"jsonrpc":"2.0","id":N,"method":M,"params":{}}`.
///
