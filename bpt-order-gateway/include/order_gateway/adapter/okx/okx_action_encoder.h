...
/// \file
/// \brief OKX WebSocket action JSON builders (pure input → JSON, no state, no I/O).
///
/// Each helper produces a boost::json::value in exactly the shape OKX's
/// trading WebSocket (`wss://ws.okx.com:8443/ws/v5/private`) accepts
/// under `op:"order" / "cancel-order" / "amend-order"`.
///
...
/// about nonces or signatures; the WS is already authenticated at login
/// time.
///
/// Contract-size and instIdCode tables are passed in by the caller
/// rather than owned by the codec — they live on the
/// OKXInstrumentsService at the adapter level and changing one doesn't
/// force a codec rebuild.
...
/// \name Lookup tables
/// Keys are OKX `instId` strings ("BTC-USDT", "BTC-USDT-SWAP", etc.).
/// @{
...
///
/// \param req_id Monotonic counter for matching the WS ack back to the
///               adapter's pending-request tracking (OKX echoes it as
...
///
/// OKX's amend endpoint doesn't require an id field in the envelope —
/// exec reports for the amended order come through the same `orders`
