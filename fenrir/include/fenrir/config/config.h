...
struct AeronConfig {
    ...
    ygg::config::StreamConfig order{"aeron:ipc", 0};             // Fenrir → Heimdall
    ygg::config::StreamConfig exec_report{"aeron:ipc", 0};       // Heimdall → Fenrir (ExecutionReport)
    ygg::config::StreamConfig heartbeat{"aeron:ipc", 0};         // Heimdall → Fenrir (OrderGatewayHeartbeat)
    ygg::config::StreamConfig account_snapshot{"aeron:ipc", 0};  // Heimdall → Fenrir (AccountSnapshot id=27)
    // Surtr vol surface streams (optional — stream_id 0 means vol surface client is not started)
    ygg::config::StreamConfig vol_surface{"aeron:ipc", 0};   // Surtr → Fenrir (VolSurface id=21)
    ygg::config::StreamConfig surtr_status{"aeron:ipc", 0};  // SurtrHeartbeat id=22, SurtrReady id=23
...
struct ScheduleConfig {
    ...
    // Bitmask of exchanges Fenrir expects Sindri to have loaded.
    // bit0=BINANCE(0x01), bit1=OKX(0x02), bit2=HYPERLIQUID(0x04).
    // If RefDataReady.exchangesLoaded is missing any bit set here, Fenrir halts.
