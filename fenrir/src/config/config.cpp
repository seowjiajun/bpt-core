...
AppConfig AppConfig::load(const std::string& path) {
    ...
    if (auto v = (*a)["pub_poll_interval_ms"].value<int64_t>())
        app_cfg.aeron.pub_poll_interval_ms = static_cast<int>(*v);

    const auto* f = cfg["fenrir"].as_table();
    if (!f)
        throw std::runtime_error("Missing 'fenrir' block in config");

    app_cfg.fenrir.correlation_id = static_cast<uint64_t>((*f)["correlation_id"].value<int64_t>().value_or(2001));
    ...
    auto& sc = app_cfg.fenrir.strategy;
