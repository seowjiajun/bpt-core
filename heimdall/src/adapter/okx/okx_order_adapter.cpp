...
std::string OKXOrderAdapter::https_request(const std::string& method, const std::string& path,
                                           const std::string& /*body*/, bool /*signed_req*/) {
    ...
    http::request<http::string_body> req(method == "GET" ? http::verb::get : http::verb::post, path,
                                         11);
    req.set(http::field::host, cfg_.rest_host);
    req.set(http::field::user_agent, "heimdall/0.1");
    if (cfg_.testnet) req.set("x-simulated-trading", "1");
...
void OKXOrderAdapter::fetch_inst_id_codes() {
    ...
    for (const auto& inst_type : inst_types) {
        try {
            std::string resp =
                https_request("GET", "/api/v5/public/instruments?instType=" + inst_type);
            auto root = json::parse(resp);
            if (!root.is_object()) continue;
            auto data_it = root.as_object().find("data");
            if (data_it == root.as_object().end() || !data_it->value().is_array()) continue;
            bool is_contract_type =
                inst_type == "SWAP" || inst_type == "FUTURES" || inst_type == "OPTION";
            ...
            spdlog::warn("[Heimdall] OKXOrderAdapter: fetch_inst_id_codes({}) failed: {}",
                         inst_type, e.what());
        }
    }
    spdlog::info("[Heimdall] OKXOrderAdapter: loaded {} instIdCodes, {} contract sizes from REST",
                 inst_id_codes_.size(), contract_sizes_.size());
...
std::string OKXOrderAdapter::build_login_msg() const {
    uint64_t ts_s = static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::seconds>(
                                              std::chrono::system_clock::now().time_since_epoch())
                                              .count());
...
void OKXOrderAdapter::handle_message(const std::string& payload, uint64_t recv_ns) {
    spdlog::info("[Heimdall] OKXOrderAdapter WS rx: {}", payload.substr(0, 500));
    ...
    if (auto eit = obj.find("event"); eit != obj.end()) {
        ...
        if (event == "error") {
            ...
            spdlog::error("[Heimdall] OKXOrderAdapter: error event code={} msg={}", code, msg);
        } else {
            spdlog::info("[Heimdall] OKXOrderAdapter: event={}", event);
        }
        if (event == "login") {
            spdlog::info("[Heimdall] OKXOrderAdapter: login successful");
            ...
            if (ws_send_) ws_send_(json::serialize(sub_msg));
        }
        return;
    }
    ...
    if (auto op_it = obj.find("op"); op_it != obj.end()) {
        if (std::string(op_it->value().as_string()) == "order") {
            ...
            if (data_it == obj.end() || !data_it->value().is_array()) return;
            for (const auto& item : data_it->value().as_array())
    ...
    auto data_it = obj.find("data");
    if (arg_it == obj.end() || data_it == obj.end()) return;
    if (!data_it->value().is_array() || data_it->value().as_array().empty()) return;
...
void OKXOrderAdapter::connect_and_run() {
    ...
    spdlog::info("[Heimdall] OKXOrderAdapter connecting {}:{}{}", cfg_.ws_host, cfg_.ws_port,
                 cfg_.ws_path);
    ...
    // All writes must be text frames — OKX requires text-frame "ping" keepalives
    // and also expects JSON messages as text. Beast defaults to binary.
    ws->text(true);
    ...
    // Disable Beast's idle timeout AFTER handshake — we manage ping/pong manually.
    ...
    spdlog::info("[Heimdall] OKXOrderAdapter connected, waiting for login");
    ...
    try {
        ...
        while (!stop_flag_.load(std::memory_order_relaxed)) {
            // Ping every 15s to avoid OKX silent disconnect.
            auto now = std::chrono::steady_clock::now();
            if (now - last_ping >= std::chrono::seconds(15)) {
                {
                    std::lock_guard<std::mutex> lk(send_mu_);
                    if (ws_send_) ws_send_("ping");
                }
                last_ping = now;
            }
            ...
            if (msg == "ping") {
                // OKX server-initiated ping — must respond or connection closes after 30s
                std::lock_guard<std::mutex> lk(send_mu_);
                if (ws_send_) ws_send_("pong");
                continue;
            }
...
void OKXOrderAdapter::send_new_order(const bifrost::protocol::NewOrder& order) {
    ...
    bool is_perp = exchange_symbol.find("-SWAP") != std::string::npos ||
                   exchange_symbol.find("-FUTURES") != std::string::npos;
    ...
    try {
        ...
        spdlog::error("[Heimdall] OKXOrderAdapter: send_new_order failed: {}", e.what());
        emit_rejection();
    }
}

void OKXOrderAdapter::send_cancel(const bifrost::protocol::CancelOrder& cancel,
    ...
    if (ws_send_) {
        try {
            ...
            spdlog::error("[Heimdall] OKXOrderAdapter: send_cancel failed: {}", e.what());
        }
    }
}

void OKXOrderAdapter::send_cancel_all(uint64_t instrument_id) {
    spdlog::warn("[Heimdall] OKXOrderAdapter: send_cancel_all called instrument_id={}",
                 instrument_id);
}

void OKXOrderAdapter::send_modify(const bifrost::protocol::ModifyOrder& modify,
                                  const std::string& native_symbol) {
    ...
    if (ws_send_) {
        try {
            ...
            spdlog::error("[Heimdall] OKXOrderAdapter: send_modify failed: {}", e.what());
        }
    }
}
...
static boost::beast::http::request<boost::beast::http::string_body> okx_signed_get(
    const std::string& host, const std::string& path, const std::string& api_key,
    const std::string& secret_key, const std::string& passphrase, bool testnet) {
    namespace http = boost::beast::http;
    const uint64_t ts_s =
    ...
    if (testnet) req.set("x-simulated-trading", "1");
    return req;
}
...
AccountSnapshotData OKXOrderAdapter::fetch_account_snapshot(uint64_t correlation_id) {
    ...
    auto do_get = [&](const std::string& path) -> std::string {
        ...
        auto req =
            okx_signed_get(cfg_.rest_host, path, api_key_, secret_key_, passphrase_, cfg_.testnet);
        http::write(stream, req);
        beast::flat_buffer buf;
        http::response<http::string_body> res;
        http::read(stream, buf, res);
    ...
    try {
        ...
        if (bal_j.is_object() && bal_j.as_object().contains("data") &&
            bal_j.as_object().at("data").is_array()) {
            ...
            if (!data.empty() && data[0].is_object()) {
                ...
                if (d.contains("totalEq"))
                    snap.total_equity_e8 = static_cast<int64_t>(
                        std::round(std::stod(std::string(d.at("totalEq").as_string())) * 1e8));
                ...
                if (d.contains("details") && d.at("details").is_array()) {
                    for (const auto& detail : d.at("details").as_array()) {
                        ...
                        if (de.contains("ccy") && std::string(de.at("ccy").as_string()) == "USDT" &&
                            ...
                            snap.available_balance_e8 = static_cast<int64_t>(std::round(
                                std::stod(std::string(de.at("availBal").as_string())) * 1e8));
                    }
                }
            }
        }
    } catch (const std::exception& e) {
        spdlog::warn("[Heimdall] OKXOrderAdapter: failed to fetch balance: {}", e.what());
    }
            ...
            for (const auto& p : pos_j.as_object().at("data").as_array()) {
                ...
                if (po.contains("avgPx") && !po.at("avgPx").as_string().empty())
                    ap.avg_entry_price_e8 = static_cast<int64_t>(
                        std::round(std::stod(std::string(po.at("avgPx").as_string())) * 1e8));
                if (po.contains("upl") && !po.at("upl").as_string().empty())
                    ap.unrealized_pnl_e8 = static_cast<int64_t>(
                        std::round(std::stod(std::string(po.at("upl").as_string())) * 1e8));
                snap.positions.push_back(std::move(ap));
            }
        }
    } catch (const std::exception& e) {
        spdlog::warn("[Heimdall] OKXOrderAdapter: failed to fetch positions: {}", e.what());
    }

    spdlog::info(
        "[Heimdall] OKXOrderAdapter: account snapshot fetched — balance={:.2f} positions={}",
