...
void OrderProcessor::on_new_order(const bifrost::protocol::NewOrder& order) {
    ...
    // exchange_symbol is resolved by Fenrir before publishing the NewOrder
...
void OrderProcessor::on_cancel(const bifrost::protocol::CancelOrder& cancel) {
...
void OrderProcessor::on_cancel_all(const bifrost::protocol::CancelAll& msg) {
...
void OrderProcessor::on_modify(const bifrost::protocol::ModifyOrder& modify) {
...
void OrderProcessor::check_stale_orders(uint64_t stale_timeout_ns) {
    ...
    state_mgr_.check_stale(cur_ns, stale_timeout_ns, [&](const OrderState& st) {
        ...
        // Publish a synthetic CANCELLED exec report so Fenrir can reconcile
        // its position and open-order book without waiting for the exchange.
        exec_pub_.publish(st.order_id,
                          st.exchange_order_id,
                          st.exchange_id,
                          st.instrument_id,
                          bifrost::protocol::ExecStatus::CANCELLED,
                          st.side,
                          st.order_type,
                          st.price,
                          st.filled_qty,
                          st.remaining_qty,
                          bifrost::protocol::RejectReason::OK,
                          0,
                          bifrost::protocol::FeeCurrency::USDT,
                          cur_ns,
                          cur_ns);
...
adapter::IOrderAdapter* OrderProcessor::find_adapter(bifrost::protocol::ExchangeId::Value id) const {
...
OrderLifecycle OrderProcessor::exec_status_to_lifecycle(bifrost::protocol::ExecStatus::Value status) {
    using ES = bifrost::protocol::ExecStatus;
    using OL = OrderLifecycle;
    switch (status) {
        case ES::ACKED:
...
const char* OrderProcessor::exchange_str(bifrost::protocol::ExchangeId::Value id) {
    using EX = bifrost::protocol::ExchangeId;
    switch (id) {
        case EX::BINANCE:
            return "BINANCE";
