...
#ifndef _BIFROST_PROTOCOL_SINDRIERROR_CXX_H_
#define _BIFROST_PROTOCOL_SINDRIERROR_CXX_H_
...
public:
    static const std::uint16_t SBE_BLOCK_LENGTH = static_cast<std::uint16_t>(18);
    ...
    RefDataError() = default;

    RefDataError(char* buffer,
    ...
    RefDataError(char* buffer, const std::uint64_t bufferLength)
        : RefDataError(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion()) {}

    RefDataError(char* buffer,
                const std::uint64_t bufferLength,
                const std::uint64_t actingBlockLength,
                const std::uint64_t actingVersion)
        : RefDataError(buffer, 0, bufferLength, actingBlockLength, actingVersion) {}
    ...
    RefDataError& wrapForEncode(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    RefDataError& wrapAndApplyHeader(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
    ...
    RefDataError& wrapForDecode(char* buffer,
                               const std::uint64_t offset,
                               const std::uint64_t actingBlockLength,
                               const std::uint64_t actingVersion,
                               const std::uint64_t bufferLength) {
    ...
    SBE_NODISCARD std::uint64_t decodeLength() const {
        RefDataError skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    RefDataError& timestampNs(const std::uint64_t value) SBE_NOEXCEPT {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
        std::memcpy(m_buffer + m_offset + 0, &val, sizeof(std::uint64_t));
        return *this;
    }

    SBE_NODISCARD static const char* errorTypeMetaAttribute(const MetaAttribute metaAttribute) SBE_NOEXCEPT {
        switch (metaAttribute) {
            case MetaAttribute::PRESENCE:
                return "required";
            default:
    ...
    static SBE_CONSTEXPR std::uint16_t errorTypeId() SBE_NOEXCEPT { return 2; }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t errorTypeSinceVersion() SBE_NOEXCEPT { return 0; }

    SBE_NODISCARD bool errorTypeInActingVersion() SBE_NOEXCEPT { return true; }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t errorTypeEncodingOffset() SBE_NOEXCEPT { return 8; }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t errorTypeEncodingLength() SBE_NOEXCEPT { return 1; }

    SBE_NODISCARD std::uint8_t errorTypeRaw() const SBE_NOEXCEPT {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 8, sizeof(std::uint8_t));
        return (val);
    }

    SBE_NODISCARD RefDataErrorType::Value errorType() const {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 8, sizeof(std::uint8_t));
        return RefDataErrorType::get((val));
    }

    RefDataError& errorType(const RefDataErrorType::Value value) SBE_NOEXCEPT {
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::size_t exchangeIdEncodingOffset() SBE_NOEXCEPT { return 9; }
    ...
    RefDataError& exchangeId(const ExchangeId::Value value) SBE_NOEXCEPT {
        std::uint8_t val = (value);
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::size_t instrumentIdEncodingOffset() SBE_NOEXCEPT { return 10; }
    ...
    RefDataError& instrumentId(const std::uint64_t value) SBE_NOEXCEPT {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    friend std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& builder,
                                                         const RefDataError& _writer) {
        RefDataError writer(_writer.m_buffer,
                           _writer.m_offset,
