...
#ifndef _BIFROST_PROTOCOL_MDSERVICEHEARTBEAT_CXX_H_
#define _BIFROST_PROTOCOL_MDSERVICEHEARTBEAT_CXX_H_
...
public:
    ...
    MdServiceHeartbeat() = default;

    MdServiceHeartbeat(char* buffer,
    ...
    MdServiceHeartbeat(char* buffer, const std::uint64_t bufferLength)
        : MdServiceHeartbeat(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion()) {}

    MdServiceHeartbeat(char* buffer,
                       const std::uint64_t bufferLength,
                       const std::uint64_t actingBlockLength,
                       const std::uint64_t actingVersion)
        : MdServiceHeartbeat(buffer, 0, bufferLength, actingBlockLength, actingVersion) {}
    ...
    MdServiceHeartbeat& wrapForEncode(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MdServiceHeartbeat& wrapAndApplyHeader(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
    ...
    MdServiceHeartbeat& wrapForDecode(char* buffer,
                                      const std::uint64_t offset,
                                      const std::uint64_t actingBlockLength,
                                      const std::uint64_t actingVersion,
                                      const std::uint64_t bufferLength) {
    ...
    SBE_NODISCARD std::uint64_t decodeLength() const {
        MdServiceHeartbeat skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    MdServiceHeartbeat& timestampNs(const std::uint64_t value) SBE_NOEXCEPT {
    ...
    static SBE_CONSTEXPR std::uint16_t seqNumId() SBE_NOEXCEPT { return 2; }
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::size_t seqNumEncodingOffset() SBE_NOEXCEPT { return 8; }
    ...
    MdServiceHeartbeat& seqNum(const std::uint64_t value) SBE_NOEXCEPT {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    friend std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& builder,
                                                         const MdServiceHeartbeat& _writer) {
        MdServiceHeartbeat writer(_writer.m_buffer,
                                  _writer.m_offset,
        ...
        builder << '{';
        builder << R"("Name": "MdServiceHeartbeat", )";
