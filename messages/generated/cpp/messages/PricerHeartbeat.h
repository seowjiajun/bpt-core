...
#ifndef _BIFROST_PROTOCOL_PRICERHEARTBEAT_CXX_H_
#define _BIFROST_PROTOCOL_PRICERHEARTBEAT_CXX_H_
...
public:
    ...
    PricerHeartbeat() = default;

    PricerHeartbeat(char* buffer,
    ...
    PricerHeartbeat(char* buffer, const std::uint64_t bufferLength)
        : PricerHeartbeat(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion()) {}

    PricerHeartbeat(char* buffer,
                   const std::uint64_t bufferLength,
                   const std::uint64_t actingBlockLength,
                   const std::uint64_t actingVersion)
        : PricerHeartbeat(buffer, 0, bufferLength, actingBlockLength, actingVersion) {}
    ...
    PricerHeartbeat& wrapForEncode(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    PricerHeartbeat& wrapAndApplyHeader(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
    ...
    PricerHeartbeat& wrapForDecode(char* buffer,
                                  const std::uint64_t offset,
                                  const std::uint64_t actingBlockLength,
                                  const std::uint64_t actingVersion,
                                  const std::uint64_t bufferLength) {
    ...
    SBE_NODISCARD std::uint64_t decodeLength() const {
        PricerHeartbeat skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    PricerHeartbeat& timestampNs(const std::uint64_t value) SBE_NOEXCEPT {
    ...
    PricerHeartbeat& seqNum(const std::uint64_t value) SBE_NOEXCEPT {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    friend std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& builder,
                                                         const PricerHeartbeat& _writer) {
        PricerHeartbeat writer(_writer.m_buffer,
                              _writer.m_offset,
        ...
        builder << '{';
        builder << R"("Name": "PricerHeartbeat", )";
