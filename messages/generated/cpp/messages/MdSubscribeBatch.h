...
public:
    ...
    MdSubscribeBatch &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    #pragma GCC diagnostic pop
        ...
        static SBE_CONSTEXPR std::uint16_t depthId() SBE_NOEXCEPT
        {
            return 4;
        }

        SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t depthSinceVersion() SBE_NOEXCEPT
        {
            return 0;
        }

        SBE_NODISCARD bool depthInActingVersion() SBE_NOEXCEPT
        {
            return true;
        }

        SBE_NODISCARD static SBE_CONSTEXPR std::size_t depthEncodingOffset() SBE_NOEXCEPT
        {
            return 40;
        }

        static SBE_CONSTEXPR std::uint8_t depthNullValue() SBE_NOEXCEPT
        {
            return SBE_NULLVALUE_UINT8;
        }

        static SBE_CONSTEXPR std::uint8_t depthMinValue() SBE_NOEXCEPT
        {
            return static_cast<std::uint8_t>(0);
        }

        static SBE_CONSTEXPR std::uint8_t depthMaxValue() SBE_NOEXCEPT
        {
            return static_cast<std::uint8_t>(254);
        }

        static SBE_CONSTEXPR std::size_t depthEncodingLength() SBE_NOEXCEPT
        {
            return 1;
        }
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MdSubscribeBatch &_writer)
{
    MdSubscribeBatch writer(
        _writer.m_buffer,
        _writer.m_offset,
