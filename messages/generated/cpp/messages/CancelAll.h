...
#ifndef _BIFROST_PROTOCOL_CANCELALL_CXX_H_
#define _BIFROST_PROTOCOL_CANCELALL_CXX_H_
...
public:
    ...
    CancelAll(char* buffer, const std::uint64_t bufferLength)
        : CancelAll(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion()) {}

    CancelAll(char* buffer,
              const std::uint64_t bufferLength,
              const std::uint64_t actingBlockLength,
              const std::uint64_t actingVersion)
        : CancelAll(buffer, 0, bufferLength, actingBlockLength, actingVersion) {}
    ...
    CancelAll& wrapForEncode(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    CancelAll& wrapAndApplyHeader(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
    ...
    CancelAll& wrapForDecode(char* buffer,
                             const std::uint64_t offset,
                             const std::uint64_t actingBlockLength,
                             const std::uint64_t actingVersion,
                             const std::uint64_t bufferLength) {
    ...
    SBE_NODISCARD std::uint64_t decodeLength() const {
        CancelAll skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    CancelAll& exchangeId(const ExchangeId::Value value) SBE_NOEXCEPT {
    ...
    CancelAll& instrumentId(const std::uint64_t value) SBE_NOEXCEPT {
    ...
    CancelAll& timestampNs(const std::uint64_t value) SBE_NOEXCEPT {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    friend std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& builder,
                                                         const CancelAll& _writer) {
        CancelAll writer(_writer.m_buffer,
                         _writer.m_offset,
