...
#ifndef _BIFROST_PROTOCOL_MDTRADE_CXX_H_
#define _BIFROST_PROTOCOL_MDTRADE_CXX_H_
...
public:
    ...
    MdTrade(char* buffer, const std::uint64_t bufferLength)
        : MdTrade(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion()) {}

    MdTrade(char* buffer,
            const std::uint64_t bufferLength,
            const std::uint64_t actingBlockLength,
            const std::uint64_t actingVersion)
        : MdTrade(buffer, 0, bufferLength, actingBlockLength, actingVersion) {}
    ...
    MdTrade& wrapForEncode(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
        m_buffer = buffer;
        m_bufferLength = bufferLength;
        m_offset = offset;
    ...
    MdTrade& wrapAndApplyHeader(char* buffer, const std::uint64_t offset, const std::uint64_t bufferLength) {
    ...
    MdTrade& wrapForDecode(char* buffer,
                           const std::uint64_t offset,
                           const std::uint64_t actingBlockLength,
                           const std::uint64_t actingVersion,
                           const std::uint64_t bufferLength) {
    ...
    SBE_NODISCARD std::uint64_t decodeLength() const {
        MdTrade skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
    ...
    MdTrade& timestampNs(const std::uint64_t value) SBE_NOEXCEPT {
    ...
    MdTrade& instrumentId(const std::uint64_t value) SBE_NOEXCEPT {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    static SBE_CONSTEXPR std::uint16_t priceId() SBE_NOEXCEPT { return 3; }
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::size_t priceEncodingOffset() SBE_NOEXCEPT { return 16; }
    ...
    MdTrade& price(const double value) SBE_NOEXCEPT {
        union sbe_double_as_uint_u val;
        val.fp_value = value;
        val.uint_value = SBE_LITTLE_ENDIAN_ENCODE_64(val.uint_value);
    ...
    static SBE_CONSTEXPR std::uint16_t qtyId() SBE_NOEXCEPT { return 4; }
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::size_t qtyEncodingOffset() SBE_NOEXCEPT { return 24; }
    ...
    MdTrade& qty(const double value) SBE_NOEXCEPT {
        union sbe_double_as_uint_u val;
        val.fp_value = value;
        val.uint_value = SBE_LITTLE_ENDIAN_ENCODE_64(val.uint_value);
    ...
    static SBE_CONSTEXPR std::uint16_t sideId() SBE_NOEXCEPT { return 5; }
    ...
    SBE_NODISCARD static SBE_CONSTEXPR std::size_t sideEncodingOffset() SBE_NOEXCEPT { return 32; }
    ...
    SBE_NODISCARD TradeSide::Value side() const {
        std::uint8_t val;
        std::memcpy(&val, m_buffer + m_offset + 32, sizeof(std::uint8_t));
        return TradeSide::get((val));
    }

    MdTrade& side(const TradeSide::Value value) SBE_NOEXCEPT {
    ...
    static SBE_CONSTEXPR std::uint16_t seqNumId() SBE_NOEXCEPT { return 6; }
    ...
    MdTrade& seqNum(const std::uint64_t value) SBE_NOEXCEPT {
        std::uint64_t val = SBE_LITTLE_ENDIAN_ENCODE_64(value);
    ...
    friend std::basic_ostream<CharT, Traits>& operator<<(std::basic_ostream<CharT, Traits>& builder,
                                                         const MdTrade& _writer) {
        MdTrade writer(_writer.m_buffer,
                       _writer.m_offset,
