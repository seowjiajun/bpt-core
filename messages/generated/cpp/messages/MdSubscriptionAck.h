...
#ifndef _BPT_MESSAGES_MDSUBSCRIPTIONACK_CXX_H_
#define _BPT_MESSAGES_MDSUBSCRIPTIONACK_CXX_H_
...
public:
    ...
    MdSubscriptionAck &wrapForDecode(
        char *buffer,
        const std::uint64_t offset,
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion,
    ...
    static SBE_CONSTEXPR std::uint16_t ackStatusId() SBE_NOEXCEPT
    {
        return 5;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::uint64_t ackStatusSinceVersion() SBE_NOEXCEPT
    {
        return 0;
    }

    SBE_NODISCARD bool ackStatusInActingVersion() SBE_NOEXCEPT
    {
        return true;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t ackStatusEncodingOffset() SBE_NOEXCEPT
    {
        return 32;
    }

    SBE_NODISCARD static SBE_CONSTEXPR std::size_t ackStatusEncodingLength() SBE_NOEXCEPT
    {
        return 1;
    }
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, const MdSubscriptionAck &_writer)
{
    MdSubscriptionAck writer(
        _writer.m_buffer,
        _writer.m_offset,
