...
public:
    ...
    void add(const model::Instrument& instrument);
    void update(const model::Instrument& instrument);
    ...
    void for_each(const std::function<void(const model::Instrument&)>& fn) const;

    // Kept for backward compatibility where a full copy is genuinely needed.
    [[nodiscard]] std::vector<model::Instrument> getAll() const;

private:
    mutable std::shared_mutex mutex_;
    std::unordered_map<uint64_t, model::Instrument> instruments_by_uid_;
