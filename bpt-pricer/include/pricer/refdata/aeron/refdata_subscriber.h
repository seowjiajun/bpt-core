...
/// \file
/// Aeron-backed concrete for api::RefdataSubscriber. Templated on the
/// Handler — in prod the Handler is `PricerService` and per-instrument
/// dispatch is direct (no std::function indirection).
///
/// Reads snapshot (stream 1001) and deltas (stream 1002) from
/// bpt-refdata; can also publish a control-stream snapshot request on
/// demand.
...
private:
    ...
    void on_delta_fragment(::aeron::AtomicBuffer& buffer,
        ...
        if (delta.updateType() == DeltaUpdateType::REMOVE) {
            if (handler_ != nullptr) [[likely]]
                handler_->on_refdata_remove(delta.instrumentId());
            return;
        }
